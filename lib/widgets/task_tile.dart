...
class TaskTile extends ConsumerWidget {
  ...
  Expanded incompleteIcon(String taskName, String type, WidgetRef ref) {
        ...
        child: IconButton(
            ...
            onPressed: () =>
                {ref.read(asyncTasksProvider.notifier).markIncomplete(taskName,type)}));
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final now = DateTime.now();
    final int daysLeft = -(taskData.lastDone.copyWith(year: now.year, month: now.month, day: now.day).difference(taskData.lastDone).inDays - taskTypes[taskData.type]!);
    final Color color = daysLeft.sign > 0 ? Colors.green.withOpacity(0.15) : daysLeft.sign == 0 ? Colors.yellow.withOpacity(0.15) : Colors.red.withOpacity(0.15);
    ...
    return Container(
        ...
        child: Row(children: <Widget>[
          textWidget,
          Expanded(flex: 2, child: Avatar(16,
                                    who: taskData.who,
